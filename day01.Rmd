...
Here, we are assigning more than a single value to variables `a` and `b` using the *concatenate* function `c()`. Most functions start with the function name followed by parentheses that encompass parameters passed to the function. We'll be making use of many more functions later in this course.
...
You’ll notice that the `col` and `col2` columns are stored as numeric (i.e. as doubles) and not as integer. There is some inconsistency in R’s characterization of data type. Here, numeric represents double whereas an integer datatype would display integer. For example:
...
Try to limit the line length to 80 characters. You can add an 80 character vertical line to you code editor via **Tools >> Global Options >> Code** and the Display tab.
